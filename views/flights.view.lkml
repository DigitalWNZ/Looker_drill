...
view: flights {
  ...
  measure: count_for_demo{
    type: count
    # drill from chart and table data may leads to different chart
    drill_fields: [total_arrival_actual,airline]
    link: {
      label: "Show as demo"
      url: "
      {% assign vis_config = '{
  ...
  #   # https://community.looker.com/explores-36/how-to-get-visualization-configurations-for-custom-drilling-20761
  #   # shows how to define the viz config
  ...
  measure: count_for_stack{
    ...
    link: {
      ...
      {{ link }}&vis_config={{ vis_config | encode_uri }}&sorts=flights.airline+asc,flights.departure_state+asc&pivots=flights.departure_state&limit=500&column_limit=15"
    } # NOTE the &pivots=
  }
